...
class PANPPCEModuleLoss(BaseTextRecogModuleLoss):
    ...
    def forward(self, outputs: torch.Tensor,
        ...
        if self.reduction == 'none' and self.dictionary.padding_idx is not None:
            mask = (targets != self.dictionary.padding_idx).to(loss_ce.device)
            loss_ce = torch.sum(loss_ce * mask.float(), dim=1) / (
                    torch.sum(mask.float(), dim=1) + 1e-6)
