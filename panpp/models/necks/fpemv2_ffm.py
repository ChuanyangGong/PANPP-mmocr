...
class FPEMv2(BaseModule):
    """FPN-like feature fusion module in PANPP.
    ...
    def forward(self, c2: torch.Tensor, c3: torch.Tensor, c4: torch.Tensor,
        ...
        """
        # upsample
        c4_ = self.up_add1(self._upsample_add(c5, c4))  # c4 shape
        c3_ = self.up_add2(self._upsample_add(c4_, c3))
        c2_ = self.up_add3(self._upsample_add(c3_, c2))

        # downsample
        c3_ = self.down_add1(self._upsample_add(c3_, c2_))
        c4_ = self.down_add2(self._upsample_add(c4_, c3_))
        c5_ = self.down_add3(self._upsample_add(c5, c4_))  # c4 / 2
...
class SeparableConv2d(BaseModule):
    ...
    convolution and pointwise convolution. This code is from https://github.com
    /open-mmlab/mmocr.
...
class FPEMv2_FFM(BaseModule):
    ...
    def __init__(
        ...
        for _ in range(fpem_repeat):
            self.fpems.append(FPEMv2(conv_out))
